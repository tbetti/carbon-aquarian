...
import { setIsVisible, setShowConfetti } from '../slices/resultsSlice'
...
export function ResultCard({ result, walletAddress }: ResultCardProps) {
  const { isVisible, showConfetti } = useSelector((s: RootState) => s.results)
  const dispatch = useDispatch<AppDispatch>()

  useEffect(() => {
    // Fade-in animation
    const timer = setTimeout(() => dispatch(setIsVisible(true)), 50)

    // Confetti effect
    const confettiTimer = setTimeout(() => {
      dispatch(setShowConfetti(true))
      setTimeout(() => dispatch(setShowConfetti(false)), 1000)
    }, 250)
        ...
        <CardContent className="px-10 py-10 space-y-6">
          ...
          <div className="space-y-3 pt-2">
            <div className="flex items-center gap-3 text-[#667085]">
              ...
              <span>
                Sent to: <span className="text-[#1A1F2C] font-medium">{truncateAddress(walletAddress)}</span>
              </span>
            </div>

            <div className="flex items-center gap-3">
              <span className="text-xl">🔗</span>
              <Button variant="link" className="p-0 h-auto text-[#00A884] hover:text-[#00A884]/80 hover:no-underline" asChild>
                <a
